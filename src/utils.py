...
def get_V_jl(x, labels, N, K):
    x = x.squeeze()
    temp = np.zeros((N, K))
    index_cluster = labels[x]
    temp[(x, index_cluster)] = 1
    temp = temp.sum(0)
    return temp


def get_fair_accuracy(U, V, labels, N, K):
    # pdb.set_trace()
    V_j = np.array([get_V_jl(x, labels, N, K) for x in V])
    balance = np.zeros(K)
    J = len(V)
    for k in range(K):
        V_j_list_k = V_j[:, k].copy()
...
def get_fair_accuracy_proportional(U, V, labels, N, K):
    V_j = np.array([get_V_jl(x, labels, N, K) for x in V])
    clustered_uV = V_j/sum(V_j)
    # balance = V_j_list/sum(V_j_list)
    fairness_error = np.zeros(K)
    U = np.array(U)
    for k in range(K):
        fairness_error[k] = (-U*np.log(np.maximum(clustered_uV[:, k], 1e-20))+U*np.log(U)).sum()
